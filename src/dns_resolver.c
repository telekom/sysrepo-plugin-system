...
int system_dns_resolver_create_dns_search(const char *value)
{
	...
	error = system_dns_resolver_apply_search_values(search_head);
	if (error) {
		SRPLG_LOG_ERR(PLUGIN_NAME, "system_dns_resolver_apply_search_values() error (%d)", error);
		goto error_out;
	}
...
int system_dns_resolver_create_dns_server_address(const char *value)
{
	...
	// set search values to the system
	error = system_dns_resolver_apply_server_values(servers_head);
	if (error) {
...
int system_dns_resolver_modify_dns_server_address(const char *prev_value, const char *new_value)
{
	...
	error = system_dns_resolver_apply_server_values(servers_head);
	if (error) {
		SRPLG_LOG_ERR(PLUGIN_NAME, "system_dns_resolver_apply_server_values() error (%d)", error);
		goto error_out;
	}
